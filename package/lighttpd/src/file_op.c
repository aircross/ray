...
int _get_share_relative_name(char *aFullPathName, char *aSharePath, int aShareLen)
{
    ...
    if(!strncmp(aFullPathName, "/tmp/mnt/", 9)) {
...
static int _move_file(const char *src, const char *dest, int option)
{
    ...
    cmd = (char *)__ax_calloc(1, strlen(src)*2 + strlen(dest)*2 + 64);
    ...
    if ( does_file_exist(dest) ) {
        ...
        //ret = rename(src, dest);
        sprintf(cmd, "mv -f %s %s 2>/dev/null 1>/dev/null", extend_src, extend_dest);
    }else{
        //ret = rename(src, dest);
        sprintf(cmd, "mv -f %s %s 2>/dev/null 1>/dev/null", extend_src, extend_dest);
    }
...
int _move_dir(char *dest, char *source, char *file, int mode)
{
    ...
    if((sourcedir=opendir(source_path))==NULL){
        DBG("cannot open directory = %s\n", source_path);
        ret = -1; //E_DEST; // FILE SYSTEM ERROR
        return ret;
    }

    DBG("_move_dir:1. dest = %s, src = %s, file = %s, mode = %d\n", dest, source, file, mode);
    ...
    while((entry=readdir(sourcedir))!=NULL){
        ...
        if(exists>=0 && S_ISDIR(statbuf.st_mode)){
            ...
            DBG("_move_dir:2. source = %s, dest = %s, file = %s, mode = %d\n", parent_t, path, entry->d_name, mode);
...
int op_move(INPUT *input)
{
    ...
    char    *dest_path = NULL, *dest_path_tmp = NULL, *full_dest_path = NULL;
...
*/
    ...
    pid = fork();
    if (pid < 0) {
        fprintf(stderr, "%s[%d] create thread failed, reason: %s\n", __func__, __LINE__, strerror(errno));
        exit(0);
    }
...
//        wfm_daemon_write_report_file(NULL, 0, 0, 0, -1, 0, thiscopypid);
        ...
        if ( (tmp=CGI_Find_Parameter(input, "source_file")) ) {
            while (tmp !=NULL && source_total != 0){
                ...
                DBG("dest_path_tmp : %s", dest_path_tmp);

                if (S_ISDIR(statbuf.st_mode)) {
                    // Handle Directory 
                    ret = 0;
                    DBG("_move_dir %s", full_path);
                    ...
                    if(strncmp(full_path, dest_path_tmp, SHARE_DATA_PATH_LEN) == 0) { // check the same volume
                        ...
                        cmd = (char *)__ax_calloc(1, strlen(full_path)*2 + strlen(dest_path_tmp)*2 + 64);
                        transfer_special_char((char *)full_path, extend_src);
                        transfer_special_char((char *)dest_path_tmp, extend_dest);

                        sprintf(cmd, "mv -f %s %s 2>/dev/null 1>/dev/null", full_path, dest_path_tmp);
                        system(cmd);
...
*/
                    remove(full_path);
                    DBG("_move_dir:2. delete dir = %s\n", full_path);
                } else if(!S_ISLNK(statbuf.st_mode)) {
                    ret = 0;
                    wfm_daemon_write_report_file(tmp->val, source_copyed_count, source_total_file_count, (unsigned long long)source_total_size, (int)(percent), 1, thiscopypid);
                    DBG( "_move_file: source = %s, dest = %s, mode = %d\n", full_path, dest_path_tmp, mode);
...
int _sys_delete(char *filename)
{
	...
	DIR	     *dir = NULL;
	int		ret;
	char msg[DESC_LENGTH], *path = NULL, tmp[MAX_FILE_PATH], fullpath[MAX_FILE_PATH];
	int veto_hidden = 0, i, file_count = 0;
    //DBG("sys_del_int %s","sys_start");
	...
	if (S_ISDIR(statbuf.st_mode)) {//del dir
        ...
        //DBG("rmdir_befor %s", filename);
		ret = rmdir(filename);
        //DBG("succeed_dir %d", ret);
        ...
        //DBG("else_path %s", "else_path");
#ifdef RECYCLE_EX
		ret = unlink_wrap(filename);
#else
       // DBG("filename_else %s",filename);
		ret = unlink(filename);
        //DBG("else_ret %d",ret);
#endif
...
{
    INPUT   *tmp = NULL;
    char    *path = NULL, *path_tmp = NULL, *full_path = NULL, *file_name = NULL, msg[MAX_CONN_LOG_MSG_SIZE+1]={0}, *sid = NULL;
    ...
    if( (tmp=CGI_Find_Parameter(input, "file_name")) ) {
        //DBG("File name %s", tmp->val);
        //DBG("file total_0 %d", file_total);
        if(nTrash == 1 && !strcmp(tmp->val, path)) { // cancel all
            ret = _delete_trash_all_file(full_path);
        } 
        else{
            //DBG("file total_1 %d", file_total);
            for (i=0; i<file_total; i++) {
                ...
                if (file_name){ 
                    //DBG("file_name_fina %s",file_name);
                    __ax_free(file_name);
                }
                if(tmp->next != NULL) 
                    tmp = tmp->next;
                else {
                    //DBG("tmp->next %s", tmp->next);
                    break;
                }
            }//for
        }//else
...
#endif
        struct utimbuf tbuf;
        char    msg[MAX_CONN_LOG_MSG_SIZE+1]={0};
        char    qsync_log_desc[DESC_LENGTH];
        int ret = 0;
...
{
    INPUT   *tmp=NULL;
    int     i=0, pid=0, ret=WFM2_SUCCESS, percent=0, status=WFM2_SUCCESS, copying=0;
    char    thiscopypid[256]={0};
...
int op_rename(INPUT *input)
{
    ...
    char    msg[MAX_CONN_LOG_MSG_SIZE+1]={0}, *full_dest_name = NULL, *full_source_name = NULL;
    int     ret = -1, exist = 0, euid = 0, egid = 0, isfolder = 0;
    FILE    *fp = NULL;
    struct stat statbuf;
    char    qsync_log_desc[DESC_LENGTH];
