...
int is_match(char *filename, INPUT *keyword)
{
    ...
    if (ret) total_match++;
    total_search++;
    FCGI_LOG("total_match %d", total_match);
    return ret;
}
...
int output_search_to_json(int start, int end, int sort_mode, char *sort_type, int permission)
{
    ...
    struct group    *data;
    char    *p = NULL;
    int nDate_format = 0; // Get_Date_Format_Index();
    int nTime_format = 0; // Get_Time_Format();
...
{
    INPUT *tmp = NULL;
    char *ptr = NULL, tmp_time[12], *folder = NULL, buffer[MAX_FILE_LENGTH], *full_path = NULL;
    struct tm *p;
    time_t timep_begin, timep_end;
    int total_share = 0, exist = 0, i;
...
int op_search(INPUT *input)
{
    ...
    char    *full_path = NULL, *source_tmp = NULL, *delim = "/", *sort_type = "ASC";
    int exist = 0, g_access = SHARE_NOACCESS, end = 0, sort_mode = 0, permission = NO, is_iso = 0;
    int nDate_format = 0; //Get_Date_Format_Index();
    int nTime_format = 0; //Get_Time_Format();
