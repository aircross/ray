...
#define __DEBUG_DETAIL__
    ...
    #define FCGI_LOG(fmt, ...) {static char fcgi_log_buf[256]; snprintf(fcgi_log_buf, sizeof(fcgi_log_buf), "%s(%d): "fmt"\n", __func__, __LINE__, ##__VA_ARGS__); write(2, fcgi_log_buf, strlen(fcgi_log_buf)); }
  #ifdef __DEBUG_DETAIL__
    #define FCGI_LOG_D FCGI_LOG
  #else
    #define FCGI_LOG_D(fmt, ...)
  #endif
#else
    //#define DBG(fmt, ...)
    //#define DBG_T()
