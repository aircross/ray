...
GenAximSetting()
{
    ...
    char port[256]={0}, lan_mac[256]={0};

    ezplib_get_attr_val("http_rule", 0, "port", port, sizeof(port), 0);
    xmlNewTextChild(QGenieSetting, NULL, BAD_CAST"AWebPort", port);

    ezplib_get_attr_val("lan_hwaddr_rule_default", 0, "hwaddr", lan_mac, sizeof(lan_mac), 0);
    xmlNewTextChild(QGenieSetting, NULL, BAD_CAST"AMac", lan_mac);
...
*/
    ...
    //fsync(STDOUT_FILENO);
    //fprintf(stderr, "%s %d\n", json_object_to_json_string(file_list_object), dlen);
    //sleep(2);
...
int Sysinfo_To_JSON(int total)
{
     ...
     for (i = 0; i < total; i++) {
         ...
         if (i==0) {
            ...
            url  = ":8080/cgi-bin/html/login-m.html";
         } else {
            id = "webFileManager";
            html = "cgi-bin/html/filemanager.html";
            name = "File Station";
            surl = ":443/cgi-bin/filemanager.html";
            url  = ":8080/cgi-bin/filemanager.html";
         }
...
int Share_List_To_JSON(char *path, char (*share)[MAX_FILE_LENGTH], int count, int isdraggable, char (*property)[8], char (*type)[16])
{
    ...
    for (i=0; i<count; i++) {
        ...
        else if (!strcmp(path, "/Qsync")){
                if (!strcmp(share[i], q_dir_1) || !strcmp(share[i],q_dir_2) || !strcmp(share[i],q_dir_3) || !strcmp(share[i],q_dir_4) || !strcmp(share[i],q_dir_5))
                continue;
                else
...
{
    FILE    *fp = NULL;
    unsigned long long size=0, length=0, start=0, end=0, nTell=0;
