...
void *x_malloc(size_t size)
{
        ...
        if (!ret) {
            FCGI_LOG("Out of memory on size %d\n", (int)size);
            _exit(1);
        }
        return ret;
}
...
void put(struct cgi_state *cgi, const char *name, const char *value)
{
	...
	var->value_uc = var->value = __ax_strdup(value);
    
	unescape(var->name);
	is_url_enc = unescape(var->value);
	...
	cgi->variables = var;
    /* We added a var_name___uc variable into CGI variable list, it will be
     * automatically added into INPUT structure and can be found by
     * "CGI_Find_Parameter(var_name___uc) */
    if(is_url_enc) {
        var->value_uc = __ax_strdup(value);
    } 
}
...
int load_one_part(struct cgi_state *cgi, FILE *f, int *len, char *boundary)
{
    ...
    char *chunked_upload = get(cgi, "func");
    char *upload_root_dir = NULL, *upload_id = NULL;
    char tmp_path[2048] = "";
    FILE *file = NULL;
    unsigned size = 0, bytes = 0;
    const char *buf;
    ...
    if (chunked_upload){
        if (!strcmp(chunked_upload, "chunked_upload")) {
            ...
            upload_root_dir = get(cgi, "upload_root_dir");
            upload_id = get(cgi, "upload_id");
    ...
    if (raw_data && name && tmp_path && file) {
...
int write_one_part(struct cgi_state *cgi, FILE *f, int *len, char *boundary, char *filename, char *name, char *tmp_path, FILE *tmpfile)
{
    int ret = 1;
    int towrite = 0;
    unsigned content_len=0, content_alloc=strlen(boundary);
...
extern char **environ;
char *fcgi_getenv(char *envvarn) {
    char *envr = "";
    int varnlen = strlen(envvarn);
    int i;
    for(i = 0; ((envr = environ[i]) != NULL);i ++) {
        if(!strncmp(envvarn, envr, varnlen) && (envr[varnlen] == '=')) {
            return &envr[varnlen + 1];
        }
    }
    return NULL;
}
...
int setup_cgi(struct cgi_state *cgi)
{
	...
	if ((p = fcgi_getenv("CONTENT_LENGTH"))) {
		cgi->content_length = strtoll(p, NULL, 10);
	}
	if ((p = fcgi_getenv("REQUEST_METHOD"))) {
		cgi->got_request = 1;
		if (strcasecmp(p, "POST") == 0) {
			cgi->request_post = 1;
		}
	}
	if ((p = fcgi_getenv("QUERY_STRING"))) {
		cgi->query_string = __ax_strdup(p);
	}
	if ((p = fcgi_getenv("SCRIPT_NAME"))) {
		cgi->url = __ax_strdup(p);
		cgi->pathinfo = cgi->url;
	}
	if ((p = fcgi_getenv("CONTENT_TYPE"))) {
...
int setup(struct cgi_state *cgi)
{
	...
	if (fcgi_getenv("GATEWAY_INTERFACE")) {
		ret = setup_cgi(cgi);
	} else {
...
void destroy(struct cgi_state *cgi)
{
	while (cgi->variables) {
		...
		__ax_free(var->name);
        if(var->value_uc != var->value)
            __ax_free(var->value_uc);
		__ax_free(var->value);
