...
int op_download(INPUT *input)
{
    ...
    char    *source_file_uc = NULL, *source_path_uc = NULL;
    char    *full_download_path = NULL;
    char    *full_download_path_uc = NULL;
    INPUT	*tmp;
    ...
    if ( (tmp=CGI_Find_Parameter(input, "source_path")) ) {
        ...
        /* tmp->val_uc it must contain a value, either identical to ro URL encoded of tmp->value  */
        source_path_uc = tmp->val_uc;
    }
    ...
    if (
        ...
        if((tmp = CGI_Find_Parameter(input, "type"))) { // for media // 1:music, 2:video, 3:photo
            ...
            if(source_file_uc && source_path_uc) {
                /* We got format of ROOT_PATH = "/tmp/mnt"
                 * source_path_uc = "/USB/some_folder" 
                 * source_file_uc = "some_file.ext" 
                 * The reconstruct of real path is ROOT_PATH + source_path_uc + "/" + source_file_uc */
                full_download_path_uc = (char *)__ax_calloc(1, sizeof(ROOT_PATH) + strlen(source_file_uc) + strlen(source_path_uc) + 10);
            }
#ifdef STORAGE_V2
            if (no_check == 1)
            ...
            sprintf(full_download_path, "%s/%s", full_path, source_file);
#endif
            if(full_download_path_uc) sprintf(full_download_path_uc, ROOT_PATH"%s/%s", source_path_uc, source_file_uc);
        }
        ...
        _download_urlenc(full_download_path, source_file, full_download_path_uc, compress, 0, offset);

        if(full_download_path) __ax_free(full_download_path);
        if(full_download_path_uc) __ax_free(full_download_path_uc);
        ...
        if ( (tmp=CGI_Find_Parameter(input, "source_file")) ) {
            ...
            _download_urlenc(download_list, compressed, full_download_path_uc, COMPRESS_ZIP_ARCHIVE_ONLY, 1, 0);
...
{
    FILE    *fp = NULL;
    char    filename[BUF_SIZE], *ptr=NULL;
    char    filename_tmp[BUF_SIZE] = {0};
    char    *pVeto=NULL;
    char    *cmd = NULL;
    /*
    VETO_INFO *veto_files=NULL;
    int     veto_file_count=0, i=0, nLen=0;
    char    sTmp[BUF_SIZE]={0};
    int     uid, gid;
    char    s[BLK_SIZE];
    int     nbyte = 0;
     */
    ...
    if (compress) {
        ...
        { 
            struct stat _fstat;
            /* Check if file is exist, if yes, we can get file size here */
            if(stat(full_path, &_fstat)) {
                return -1;
            }
            // printf("Content-Length: %llu\n", (unsigned long long)(_fstat.st_size));
            if(!url_encoded_path) url_encoded_path = full_path;
            printf("Content-Disposition: attachment; filename=\"%s\"\n", filename);
            snprintf(buf, sizeof(buf), "X-Sendfile2: %s %lld-%lld\n", url_encoded_path, offset, _fstat.st_size - 1);
            printf("%s", buf);
            // printf("X-Sendfile2: %s %lld-%lld\n", full_path, offset, _fstat.st_size);
            // printf("X-Sendfile2:/tmp/mnt/Photos/IMAG0017.jpg 0-893495\n");
            printf("Content-Type: application/force-download%c%c%c%c", 13, 10, 13, 10);
        }
...
int op_viewer(INPUT *input)
{
    INPUT   *tmp=NULL;
    char    *source_file=NULL, *source_path=NULL, *full_path=NULL;
    char    *source_file_uc=NULL, *source_path_uc=NULL, *full_download_path_uc=NULL;
    ...
    int sstart = 0;
    /*
    WFM_CLIENT_INFO clientInfo;
    SHARE_LINK_EVENT_T  sShareData;
    struct stat statbuf;
    int     uid, gid;
    int     nShareLen=0, nFileLen=0;
    char    *filename=NULL, *fid=NULL, *p=NULL, *path_tmp=NULL, *path=NULL;
    ...
    */
    ...
    if(!(tmp=CGI_Find_Parameter(input, "ssid"))) {
        if ( (tmp=CGI_Find_Parameter(input, "source_path")) ) {
            ...
            if((tmp=CGI_Find_Parameter(input, "source_path___uc"))) {
                source_path_uc = tmp->val;
            } else {
                source_path_uc = source_path;
            }
        }
/*
        path_tmp = (char *)__ax_calloc(1, strlen(source_path) + 1);
...
*/
        ...
        if(format != NULL) {
            ...
            full_download_path_uc = 
                (char *)__ax_calloc(1, sizeof(ROOT_PATH) + strlen(source_path_uc) + WFM2_VIDEO_TRANSCODE_PATH_LEN + strlen(source_file_uc) + 20);
            if(!strcmp(format, WFM2_VFORMAT_MP4_240) || !strcmp(format, WFM2_VFORMAT_FLV_240)) {
                sprintf(full_download_path, "%s/%s/%s%s", 
                        full_path, WFM2_VIDEO_TRANSCODE_PATH, source_file, WFM2_TRANSCODED_FILE_POSTFIX_MP4_240);
                sprintf(full_download_path_uc, ROOT_PATH"%s/%s/%s%s", 
                        source_path_uc, WFM2_VIDEO_TRANSCODE_PATH, source_file_uc, WFM2_TRANSCODED_FILE_POSTFIX_MP4_240);
                nGot = 1;
            }
            else if(!strcmp(format, WFM2_VFORMAT_MP4_360) || !strcmp(format, WFM2_VFORMAT_FLV_360)) {
                sprintf(full_download_path, "%s/%s/%s%s", 
                        full_path, WFM2_VIDEO_TRANSCODE_PATH, source_file, WFM2_TRANSCODED_FILE_POSTFIX_MP4_360);
                sprintf(full_download_path_uc, ROOT_PATH"%s/%s/%s%s", 
                        source_path_uc, WFM2_VIDEO_TRANSCODE_PATH, source_file_uc, WFM2_TRANSCODED_FILE_POSTFIX_MP4_360);
                nGot = 1;
            }
            else if(!strcmp(format, WFM2_VFORMAT_MP4_720) || !strcmp(format, WFM2_VFORMAT_FLV_720)) {
                sprintf(full_download_path, "%s/%s/%s%s", 
                        full_path, WFM2_VIDEO_TRANSCODE_PATH, source_file, WFM2_TRANSCODED_FILE_POSTFIX_MP4_720);
                sprintf(full_download_path_uc, ROOT_PATH"%s/%s/%s%s", 
                        source_path_uc, WFM2_VIDEO_TRANSCODE_PATH, source_file_uc, WFM2_TRANSCODED_FILE_POSTFIX_MP4_720);
                nGot = 1;
            }
            else if(!strcmp(format, WFM2_VFORMAT_MP4_480) || !strcmp(format, WFM2_VFORMAT_FLV_480)) {
                sprintf(full_download_path, "%s/%s/%s%s", 
                        full_path, WFM2_VIDEO_TRANSCODE_PATH, source_file, WFM2_TRANSCODED_FILE_POSTFIX_MP4_480);
                sprintf(full_download_path_uc, ROOT_PATH"%s/%s/%s%s", 
                        source_path_uc, WFM2_VIDEO_TRANSCODE_PATH, source_file_uc, WFM2_TRANSCODED_FILE_POSTFIX_MP4_480);
                nGot = 1;
            }
            else if(!strcmp(format, WFM2_VFORMAT_MP4_1080) || !strcmp(format, WFM2_VFORMAT_FLV_1080)) {
                sprintf(full_download_path, "%s/%s/%s%s", 
                        full_path, WFM2_VIDEO_TRANSCODE_PATH, source_file, WFM2_TRANSCODED_FILE_POSTFIX_MP4_1080);
                sprintf(full_download_path_uc, ROOT_PATH"%s/%s/%s%s", 
                        source_path_uc, WFM2_VIDEO_TRANSCODE_PATH, source_file_uc, WFM2_TRANSCODED_FILE_POSTFIX_MP4_1080);
                nGot = 1;
            }
            else {
                sprintf(full_download_path, "%s/%s", full_path, source_file);
                sprintf(full_download_path_uc, ROOT_PATH"%s/%s", source_path_uc, source_file_uc);
            }
        }
        else {
            full_download_path = (char *)__ax_calloc(1, strlen(full_path) + strlen(source_file) + 6);
            full_download_path_uc = 
                (char *)__ax_calloc(1, sizeof(ROOT_PATH) + strlen(source_path_uc) + strlen(source_file_uc) + 6);
            sprintf(full_download_path, "%s/%s", full_path, source_file);
            sprintf(full_download_path_uc, ROOT_PATH"%s/%s", source_path_uc, source_file_uc);
        }
    }
    ...
    if(ssid != NULL && strcmp(mode, "normal")) {
        if(nCheck == 1) {
            if(wfm2_range_download_with_url(full_download_path, full_download_path_uc, WFM2_DL_FLAG_CHECK_MIMETYPE) < 0) nRet=-1;
        }
        else {
            if(wfm2_range_download_with_url(full_download_path, full_download_path_uc, WFM2_DL_FLAG_DEFAULT) < 0) nRet=-1;
        }
    }
