...
#error "No define brand"
    ...
    { "wan_pptp_l2tp_rule", WAN_PPTP_L2TP_RULE,
        NVRAM_NONE,
        0
    },  /* vpn_type^host^iptype[static|dhcp]^ipaddr^netmask^gateway^username^
...
#define BW_GROUP_RULE_MAX 4
    ...
    { "smbd_rule", "0^QGenie^QGenie^^0777^0777^^share^mnt^^0^0^0^cp437",
        NVRAM_NONE,
        0
    }, /* enable[0|1]^name^workgroup^description^create_mask^dir_mask^security^share_name^share_path^share_users^read_only^guest_ok^ext_acces^codepage */
...
#if defined(EZP_PROD_BRAND_SONY) || defined(EZP_SUB_BRAND_GARMIN) || defined(PLATFORM_AXA) ||defined(PLATFORM_AXM) || defined(PLATFORM_M2E)
    { "wl0_apcli_swap_basic_rule", "1^6^0",
#else
    { "wl0_apcli_swap_basic_rule", "0^6^0",
#endif
        NVRAM_NONE,
        0 
    },      /* enable^max^index */
