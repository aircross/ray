...
/* 3.0.0 */
int fw_func_2_0_9_to_3_0_0(void);
int fw_func_3_0_0_to_2_0_9(void);
/* 3.0.1 */
...
struct nvram_fw_tuple nvram_fw_table[] = {
    ...
    { "3.0.0", NVRAM_FW_3_0_0, fw_func_2_0_9_to_3_0_0, fw_func_3_0_0_to_2_0_9 },
...
static void get_ver(char *verstr, int *relver, int *alphabetaver, char *stage) {
    char *dashptr;
    char *dotptr;
    char *ahphaptr;
    char *betaptr;
    char *verstart;
    int relv = 0, alphabetav = 0;
...
non_standard_release_process(char *old_str, char *new_str) {
    int oldver = 0, oldabver = 0, newver = 0, newabver = 0;
    char oldab = 'r', newab = 'r';
    get_ver(old_str, &oldver, &oldabver, &oldab);
    get_ver(new_str, &newver, &newabver, &newab);
    printf("%s(%d) from oldver:%x oldab:%c oldabver:%d to newver:%x newab:%c newabver:%d\n", __func__, __LINE__, oldver, oldab, oldabver, newver, newab, newabver);
    if(oldab != newab && oldab != 'r' && newab != 'r') return 1;
    if(oldver != newver) return 1;
    ...
    if(newver == 0x00030007) {
        printf("%s(%d) set time zone\n");
        ezplib_replace_attr("ntp_rule", 0, "zone", "UTC0");
        ezplib_replace_attr("ntp_rule", 0, "daylight", "0");
    }

    if((oldver <= 0x00030100 && newver >= 0x00030100)&& (oldabver < 2 && newabver >= 2)) {
        nvram_default();
        nvram_commit();
    }
...
int fw_func_2_0_9_to_3_0_0(void)
{
    printf("upgrade to 3.0\n");
    return 0;
}
int fw_func_3_0_0_to_2_0_9(void)
{
    printf("downgrade from 3.0\n");
    return 0;
}
/* 3.0.1 */
int fw_func_3_0_0_to_3_0_1(void)
