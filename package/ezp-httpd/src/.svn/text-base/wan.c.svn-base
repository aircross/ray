...
_valid_wan_static(webs_t wp, char *value, struct variable *v)
{
    ...
    if (valid_range(wp, val, &wan_variables[WAN_NETMASK]) == FALSE) {
        return FALSE;
    }
...
{
    int i, max, change=0;
    char *submit_button, *not_encode_ssid, *bssid, *secmode, *channel, 
    ...
    char new_channel_rule_str[TMP_LEN], new_swap_rule_str[TMP_LEN],
         ...
         sec_wpa2_rule_str[TMP_LEN];
    char tmp[TMP_LEN], ssid[TMP_LEN],
         tmp_max[TMP_LEN], tmp_channel[TMP_LEN], tmp_extcha[TMP_LEN],
         tmp_enable[TMP_LEN],tmp_ssid[TMP_LEN], tmp_bssid[TMP_LEN], 
         tmp_secmode[TMP_LEN], tmp_wifiwan[TMP_LEN],
         tmp_key_index[TMP_LEN], tmp_key1[TMP_LEN],
         tmp_key2[TMP_LEN],tmp_key3[TMP_LEN],tmp_key4[TMP_LEN],
         tmp_keytype[TMP_LEN], tmp_encmode[TMP_LEN],
         tmp_key[TMP_LEN], tmp_crypto[TMP_LEN], cur_bssid[TMP_LEN],
...
#else
    nvram_set("wan0_device", "apcli0");
#endif

    for (i = 0; i < max; i++){

        ezplib_get_attr_val("wl0_apcli_swap_rule", i, "enable", tmp_enable, sizeof(tmp_enable), EZPLIB_USE_CLI);
        ezplib_get_attr_val("wl0_apcli_swap_rule", i, "ssid", tmp_ssid, sizeof(tmp_ssid), EZPLIB_USE_CLI);
        ezplib_get_attr_val("wl0_apcli_swap_rule", i, "bssid", tmp_bssid, sizeof(tmp_bssid), EZPLIB_USE_CLI);
        ezplib_get_attr_val("wl0_apcli_swap_rule", i, "secmode", tmp_secmode, sizeof(tmp_secmode), EZPLIB_USE_CLI);
        ezplib_get_attr_val("wl0_apcli_swap_rule", i, "wifiwan", tmp_wifiwan, sizeof(tmp_wifiwan), EZPLIB_USE_CLI);

        ezplib_get_attr_val("wl0_apcli_swap_channel_rule", i, "channel", tmp_channel, sizeof(tmp_channel), EZPLIB_USE_CLI);      
        ezplib_get_attr_val("wl0_apcli_swap_channel_rule", i, "extcha", tmp_extcha, sizeof(tmp_extcha), EZPLIB_USE_CLI);

        ezplib_get_attr_val("wl0_apcli_swap_sec_wep_rule", i, "key_index", tmp_key_index, sizeof(tmp_key_index), EZPLIB_USE_CLI);
        ezplib_get_attr_val("wl0_apcli_swap_sec_wep_rule", i, "key1", tmp_key1, sizeof(tmp_key1), EZPLIB_USE_WEB);
        ezplib_get_attr_val("wl0_apcli_swap_sec_wep_rule", i, "key2", tmp_key2, sizeof(tmp_key2), EZPLIB_USE_WEB);
        ezplib_get_attr_val("wl0_apcli_swap_sec_wep_rule", i, "key3", tmp_key3, sizeof(tmp_key3), EZPLIB_USE_WEB);
        ezplib_get_attr_val("wl0_apcli_swap_sec_wep_rule", i, "key4", tmp_key4, sizeof(tmp_key4), EZPLIB_USE_WEB);
        ezplib_get_attr_val("wl0_apcli_swap_sec_wep_rule", i, "keytype", tmp_keytype, sizeof(tmp_keytype), EZPLIB_USE_WEB);
        ezplib_get_attr_val("wl0_apcli_swap_sec_wep_rule", i, "encmode", tmp_encmode, sizeof(tmp_encmode), EZPLIB_USE_CLI);

        ezplib_get_attr_val("wl0_apcli_swap_sec_wpa_rule", i, "key", tmp_key, sizeof(tmp_key), EZPLIB_USE_WEB);
        ezplib_get_attr_val("wl0_apcli_swap_sec_wpa_rule", i, "crypto", tmp_crypto, sizeof(tmp_crypto), EZPLIB_USE_CLI);

        ezplib_get_attr_val("wl0_apcli_swap_sec_wpa2_rule", i, "key", tmp_key, sizeof(tmp_key), EZPLIB_USE_WEB);
        ezplib_get_attr_val("wl0_apcli_swap_sec_wpa2_rule", i, "crypto", tmp_crypto, sizeof(tmp_crypto), EZPLIB_USE_CLI);
        ...
        if(!strcmp(submit_button, "save")){
            ...
            if(!strcmp(bssid, tmp_bssid)){
                ...
                strcpy(secmode, tmp_secmode);
                strcpy(channel, tmp_channel);
                strcpy(extcha, tmp_extcha);
                strcpy(key_index, tmp_key_index);
                strcpy(key1, tmp_key1);
                strcpy(key, tmp_key);
                strcpy(crypto, tmp_crypto);
            } else {

                if(!strcmp(swap_rule_str, "") ){
                    snprintf(swap_rule_str, TMP_LEN, "%s^%s^%s^%s^%s", tmp_enable, tmp_ssid,
                            tmp_bssid, tmp_secmode, tmp_wifiwan);
                } else {
                    snprintf(swap_rule_str, TMP_LEN, "%s|%s^%s^%s^%s^%s",swap_rule_str, tmp_enable,
                            tmp_ssid, tmp_bssid, tmp_secmode, tmp_wifiwan);
                }

                if(!strcmp(channel_rule_str, "") ){
                    snprintf(channel_rule_str, TMP_LEN, "%s^%s", tmp_channel, tmp_extcha);
                } else {
                    snprintf(channel_rule_str, TMP_LEN, "%s|%s^%s",channel_rule_str, tmp_channel, tmp_extcha);
                }

                if(!strcmp(sec_wep_rule_str, "") ){
                    snprintf(sec_wep_rule_str, TMP_LEN, "%s^%s^%s^%s^%s^%s^%s", tmp_key_index, tmp_key1,
                            tmp_key2, tmp_key3, tmp_key4, tmp_keytype, tmp_encmode);
                } else {
                    snprintf(sec_wep_rule_str, TMP_LEN, "%s|%s^%s^%s^%s^%s^%s^%s",sec_wep_rule_str, tmp_key_index, tmp_key1,
                            tmp_key2, tmp_key3, tmp_key4, tmp_keytype, tmp_encmode);
                }

                if(!strcmp(sec_wpa_rule_str, "") ){
                    snprintf(sec_wpa_rule_str, TMP_LEN, "%s^%s", tmp_key, tmp_crypto);
                } else {
                    snprintf(sec_wpa_rule_str, TMP_LEN, "%s|%s^%s",sec_wpa_rule_str, tmp_key, tmp_crypto);
                }

                if(!strcmp(sec_wpa2_rule_str, "") ){
                    snprintf(sec_wpa2_rule_str, TMP_LEN, "%s^%s", tmp_key, tmp_crypto);
                } else {
                    snprintf(sec_wpa2_rule_str, TMP_LEN, "%s|%s^%s",sec_wpa2_rule_str, tmp_key, tmp_crypto);
                }
            }
    ...
    if(!strcmp(submit_button, "save")){
        ...
        ezplib_replace_attr("wl0_apcli_swap_basic_rule", 0, "index", "0");
        change = 1;
    } else {    // Forget button
    ...
    if (change) {
        ...
        if(!strcmp(first_wisp_swap_bssid, "")){
            system("nvram fset wl0_apcli_rule=0^^^disabled^0");
        } else {
            system("nvram fset wl0_apcli_rule=1^^^disabled^0");
        }
    ...
    for (i = 1; strlen(ezp_com_vendor_map[i].desc); i++) {
        ...
        if (ezp_com_vendor_map[i].shown) {
            ...
            for (j = 0; strlen(map_ptr[j].desc); j++)  {
                ...
                if ((map_ptr[j].Flag >= 0x2) && (strstr(map_ptr[j].desc, "HIDDEN") == NULL)) {
...
ej_wan_wwan_show_modem(int eid, webs_t wp, int argc, char_t **argv)
{
    ...
    websWrite(wp, "<option value='0' %s>%s</option>\n",
    ...
    if (nbrand == 0) {
        ...
        for (i = 0; strlen(map_ptr[i].desc); i++) {
            if ((shown == 2) && (strstr(map_ptr[i].desc, "HIDDEN") == NULL)){
                if (map_ptr[i].Flag >= 2){
                    websWrite(wp, "<option value='%d' %s>%s</option>\n", i,
                            (nmodel == i) ? "selected" : "", map_ptr[i].desc);
                }
            }
            else if ((map_ptr[i].Flag == shown) && (strstr(map_ptr[i].desc, "HIDDEN") == NULL)) {
                websWrite(wp, "<option value='%d' %s>%s</option>\n", i,
                        (nmodel == i) ? "selected" : "", map_ptr[i].desc);
            }
        }
