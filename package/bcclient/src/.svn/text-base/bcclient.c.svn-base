...
void* main_flow(void *port)
{
	...
	pthread_t recv_thread;
        printf("%s(%d)create receiver thread\n", __func__, __LINE__);
	...
	int len = TagListToBuffer(&tag_list, tag_buf, sizeof(tag_buf));
        printf("%s(%d) start process first acquire\n", __func__, __LINE__);
	...
	for ( ; ; ) {
        printf("%s(%d) process packet\n", __func__, __LINE__);
			...
			for (sock_index = 0; sock_index < r_lan_num + r_wan_num; sock_index++)
			{
				if (nic_socket_array[sock_index] > 0) ret = sendto(nic_socket_array[sock_index], send_buf, len, 0, (struct sockaddr*) &nic_sa_array[sock_index], sizeof(nic_sa_array[sock_index]));
			}
		}
	}
	else
