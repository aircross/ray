...
get_link_status(FILE *fo)
{
    ...
    char usb_plug[TMP_LEN]={0}, ether_plug[TMP_LEN]={0}, ether_connect[TMP_LEN]={0}, wisp_connect[TMP_LEN]={0}, usb_connect[TMP_LEN]={0};
    char *proto, *ip_addr;
    char ssid[TMP_LEN]={0};
    ...
    ezplib_get_attr_val("cnnt_rule", 0, "connection",
            ether_connect, sizeof(ether_connect), EZPLIB_USE_CLI);
    ezplib_get_attr_val("cnnt_rule", 1, "connection",
            wisp_connect, sizeof(wisp_connect), EZPLIB_USE_CLI);
    ezplib_get_attr_val("cnnt_rule", 2, "connection",
            usb_connect, sizeof(usb_connect), EZPLIB_USE_CLI);
    ...
    fprintf(fo, "ether_connect=%s\n", ether_connect);
    fprintf(fo, "wisp_connect=%s\n", wisp_connect);
    fprintf(fo, "usb_connect=%s\n", usb_connect);
}
...
get_meminfo(FILE *fo)
{
    ...
    p = strchr(buf, ':');
    *p = '\0';
    sscanf(p + 1, "%lu %lu%*u%*u%*u%*u", &total);
    fgets(buf, sizeof(buf), f);

    p = strchr(buf, ':');
    *p = '\0';
    sscanf(p + 1, "%lu %lu%*u%*u%*u%*u", &free);
    used=total-free;
    fprintf(fo, "Memory=%.0f\n", (double)used/(total+5000)*100);
    fclose(f);
}
