...
<html lang="en">
    ...
    function valid_password() {
        for(i=0 ; i<$("#http_passwd").val().length; i++){
            ch = $("#http_passwd").val().charAt(i).toLowerCase();
            if(ch >= '0' && ch <= '9' || ch >= 'a' && ch <= 'z' || ch >= 'A' && ch <= 'Z') {
            } else {
                alert("<% lang("Password contains characters which are non-English alphabet or numbers.\nPlease input with the combination which has only number and English alphabet with upper or lower case again."); %>");
                return false;
            }
        }
