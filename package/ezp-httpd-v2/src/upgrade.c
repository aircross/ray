...
do_upgrade(char *url, webs_t stream, int *total)
{
    ...
    if (ezp_copy_file("/usr/sbin/ezp-i2c","/tmp/ezp-i2c")) {
        printf("copy /usr/sbin/ezp-i2c /tmp/ezp-i2c failed\n");
    }
    if (chmod("/tmp/ezp-i2c", 0x0777)) {
        printf("chmod /tmp/ezp-i2c to 0777 failed\n");
    }
    if (ezp_copy_file("/sbin/ifconfig","/tmp/ifconfig")) {
        printf("copy /sbin/ifconfig /tmp/ifconfig failed\n");
    }
