...
#ifdef DEBUG
    printf("change:%d restart_wan:%d selected_swap_index:%d orig_proto:%s\n", change, restart_wan, selected_swap_index, orig_proto);
#endif
    if (change) {
        log_time(__func__, __LINE__);
...
#ifdef DEBUG
        printf("swap rule changed:%s\nfirst bssid:%s\nchannel_rule_str:%s\nsec_wpa_rule_str:%s\nsec_wpa2_rule_str:%s\n", 
                swap_rule_str, first_wisp_swap_bssid, channel_rule_str, sec_wpa_rule_str, sec_wpa2_rule_str);
#endif
...
{
    int i, num, change = 0, connect_num = 0;
    char buf[TMP_LEN], tmp[TMP_LEN];
    int64_t map = 0;
    char *cnnt_num, *name, *expand, *enable, *connection, *proto;
    char wisp_change = 0;
    char *orig_proto = nvram_get("wan0_proto");

    cnnt_num = websGetVar(wp, "cnnt_rule_num", "");
    num = atoi(cnnt_num);

    for(i=0;i<num;i++) {
        snprintf(buf, TMP_LEN, "cnnt_rule%d_name", i);
        name = websGetVar(wp, buf, "");

        snprintf(buf, TMP_LEN, "cnnt_rule%d_enable", i);
        enable = websGetVar(wp, buf, "");

        snprintf(buf, TMP_LEN, "cnnt_rule%d_expand", i);
        expand = websGetVar(wp, buf, "");

        snprintf(buf, TMP_LEN, "cnnt_rule%d_connection", i);
        connection = websGetVar(wp, buf, "");

        snprintf(buf, TMP_LEN, "cnnt_rule%d_proto", i);
        proto = websGetVar(wp, buf, "");

        snprintf(buf, TMP_LEN, "%s^%s^%s^%s^%s", expand, name, enable, connection, proto);

        int j;
        for(j=0;j<num;j++) {
            ezplib_get_attr_val("cnnt_rule", j, "name", tmp, sizeof(tmp), EZPLIB_USE_CLI);
            if(!strcmp(tmp, name)) 
                break;
        }

        ezplib_get_rule("cnnt_rule", j, tmp, TMP_LEN);

        if(!strcmp(connection, "1") && strcmp(orig_proto, proto)) {
            if(change==0) 
                config_preaction(&map, v, s, "NUM=0", "");
            change = 1;
            /* If we change from "wisp" to other proto, we need to reset our
             * channel to suto.*/
            if(!strncmp(orig_proto, "wisp", 4) && strncmp(proto, "wisp", 4)) {
                ezplib_replace_attr("wl_basic_rule", 0, "channel", "0");
            }

            ezplib_replace_rule("cnnt_rule", j, buf);

            if (!strcmp(name, "Ethernet") && !strcmp(connection, "1")) {
                connect_num++;
                ...
                ezplib_replace_attr("cnnt_rule", 1, "connection", "0");
                ezplib_replace_attr("cnnt_rule", 2, "connection", "0");
                ezplib_replace_attr("cnnt_rule", 0, "proto", proto);
            } else if (!strcmp(name, "WISP") && !strcmp(connection, "1")) {
                nvram_set("wan0_proto", proto);                
                nvram_set("wan0_ifname", "apcli0");
                ...
                ezplib_replace_attr("cnnt_rule", 1, "connection", "1");
                ezplib_replace_attr("cnnt_rule", 2, "connection", "0");
                ezplib_replace_attr("cnnt_rule", 1, "proto", proto);
            } else if (!strcmp(name, "USB") && !strcmp(connection, "1")) {
                connect_num++;                     
                ezplib_replace_attr("wl0_apcli_rule", 0, "enable", "0");
                ezplib_replace_attr("wl0_apcli_swap_rule", 0, "enable", "0");
                if (!strcmp(proto, "iphone")) {
                    nvram_set("wan0_proto", proto);
                    nvram_set("wan0_ifname", "eth0");
                } else if (!strcmp(proto, "htc")) {
                    nvram_set("wan0_proto", proto);
                    nvram_set("wan0_ifname", "usb0");
                ...
                ezplib_replace_attr("cnnt_rule", 1, "connection", "0");
                ezplib_replace_attr("cnnt_rule", 2, "connection", "1");
                ezplib_replace_attr("cnnt_rule", 2, "proto", proto);
            } 
        }
    }
#ifdef DEBUG
    printf("change:%d name:%s connection:%s\n", change, name, connection);
#endif
