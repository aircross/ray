...
# only does 64k for now
# deadc0de is added after mkimage
# squashfs
define prepare_generic_squashfs
	dd if=$(1) of=$(KDIR)/tmpfile.1
	dd of=$(1) if=$(KDIR)/tmpfile.1 bs=$(2) conv=sync
endef
...
#		$(call add_jffs2_mark,$(KDIR)/root.jffs2-$(1))
  ...
  ifeq ($(CONFIG_TARGET_ROOTFS_SQUASHFS4),y)
    define Image/mkfs/squashfs
		@mkdir -p $(TARGET_DIR)/jffs
		$(STAGING_DIR_HOST)/bin/mksquashfs4 $(TARGET_DIR) $(KDIR)/root.squashfs -nopad -noappend -root-owned -p '/dev d 755 0 0' -p '/dev/console c 600 0 0 5 1' -processors 1
		$(call Image/Build,squashfs)
    endef
  endif
...
# With mini_fo, we can use runtime overlayfs, otherwise, we need to use ramfs
# overlay
ifeq ($(CONFIG_PACKAGE_kmod-fs-mini_fo),y)
  ...
  define Image/mkfs/prepare/default
	...
	$(INSTALL_DIR) $(TARGET_DIR)/tmp
	chmod 0777 $(TARGET_DIR)/tmp
	$(INSTALL_DIR) $(TARGET_DIR)/dev/pts
	$(TOPDIR)/include/makedevlinks $(TARGET_DIR)
  endef
endif
